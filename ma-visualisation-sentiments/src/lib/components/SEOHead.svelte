<script lang="ts">
	...
	function getViewTitle(view: string): string {
		switch (view) {
			case 'charts':
				return 'Charts & Distributions';
			case 'trends':
				return 'Temporal Trends';
			case 'correlation':
				return 'Sentiment Distribution';
			case 'volume':
				return 'Article Volume Analysis';
			case 'heatmap':
				return 'Centrality Heatmap';
			case 'table':
	...
	function getViewTitleFr(view: string): string {
		switch (view) {
			case 'charts':
				return 'Graphiques et distributions';
			case 'trends':
				return 'Tendances temporelles';
			case 'correlation':
				return 'Distribution des sentiments';
			case 'volume':
				return "Analyse du volume d'articles";
			case 'heatmap':
				return 'Carte de chaleur de centralité';
			case 'table':
				return "Explorateur d'articles";
			case 'comparison':
				return 'Comparaison de modèles';
			default:
	...
	function getViewDescription(view: string): string {
		switch (view) {
			case 'charts':
				return 'sentiment distribution charts and polarity analysis';
			case 'trends':
				return 'temporal evolution of sentiment trends over time';
			case 'correlation':
				return 'cross-dimensional sentiment distribution analysis';
			case 'volume':
				return 'article publication volume and geographic distribution';
			case 'heatmap':
				return 'Islam centrality heatmap by country and year';
			case 'table':
				return 'detailed article exploration with filtering capabilities';
			case 'comparison':
				return 'AI model comparison and discrepancy analysis';
			default:
	...
	function getViewDescriptionFr(view: string): string {
		switch (view) {
			case 'charts':
				return "les graphiques de distribution des sentiments et l'analyse de polarité";
			case 'trends':
				return "l'évolution temporelle des tendances de sentiment";
			case 'correlation':
				return "l'analyse de distribution croisée des sentiments";
			case 'volume':
				return "le volume de publication d'articles et la distribution géographique";
			case 'heatmap':
				return "la carte de chaleur de centralité de l'islam par pays et année";
			case 'table':
				return "l'exploration détaillée d'articles avec capacités de filtrage";
			case 'comparison':
				return "la comparaison de modèles IA et l'analyse des divergences";
			default:
				return 'la visualisation interactive de données';
		}
	}
	...
	function getViewKeywords(view: string): string {
		switch (view) {
			case 'charts':
				return 'charts, polarity, sentiment distribution, bar charts, pie charts';
			case 'trends':
				return 'trends, temporal analysis, time series, evolution';
			case 'correlation':
				return 'correlation, cross-analysis, distribution, relationships';
			case 'volume':
				return 'volume, geographic distribution, publication trends';
			case 'heatmap':
				return 'heatmap, centrality, geographic analysis, visualization';
			case 'table':
				return 'table, article explorer, filtering, search';
			case 'comparison':
				return 'comparison, AI models, discrepancy, evaluation';
			default:
				return 'visualization, analysis, research';
		}
	}
	...
	function getViewKeywordsFr(view: string): string {
		switch (view) {
			case 'charts':
				return 'graphiques, polarité, distribution des sentiments, graphiques en barres, camemberts';
			case 'trends':
				return 'tendances, analyse temporelle, séries chronologiques, évolution';
			case 'correlation':
				return 'corrélation, analyse croisée, distribution, relations';
			case 'volume':
				return 'volume, distribution géographique, tendances de publication';
			case 'heatmap':
				return 'carte de chaleur, centralité, analyse géographique, visualisation';
			case 'table':
				return "tableau, explorateur d'articles, filtrage, recherche";
			case 'comparison':
				return 'comparaison, modèles IA, divergences, évaluation';
			default:
				return 'visualisation, analyse, recherche';
		}
	}
</script>
