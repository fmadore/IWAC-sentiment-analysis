...
import type { ComparisonData, SentimentAnalysis, DiscrepancyInfo } from '$lib/types/data';
import { getModelsFromPair } from '$lib/types/data';
import { getJournalName } from '$lib/utils';
import { comparisonMode, comparisonPair } from './datasets.svelte';
...
export const comparisonData = derived(
	...
	([$datasets, $isComparison, $pair]) => {
		...
		const modelBMap = new SvelteMap(
			$datasets[modelBId].map((article) => [article['o:id'], article])
		);
